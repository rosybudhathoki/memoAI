  ...
  return (
    <div className="grainy min-h-screen w-full flex flex-col p-6 md:p-10">
      <div className="w-full max-w-5xl mx-auto flex flex-col gap-6">
        ...
        {/* Notes Grid */}
        <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 ${notes.length === 0 ? 'justify-center' : ''}`}>

          {/* Show "no notes yet" message above New Notebook button if empty */}
          {notes.length === 0 && (
            <div className="col-span-full text-center py-4">
              <h2 className="text-xl font-semibold text-gray-700">You have no notes yet.</h2>
            </div>
          )}

          {/* New Notebook card (always visible) */}
          <Dialog>
